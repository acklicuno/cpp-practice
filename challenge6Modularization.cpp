...
/* Modularization Pseudocode

Function Prototypes:
Display Elements - 
std::vector<int> DisplayElements(char)
std::vector<int> AddElement(char)
std::vector<int> MeanElement(char)
std::vector<int> SmallestElement(char)
std::vector<int> LargestElement(char)

Main Function:
In here make sure to use if statements depending upon the user input(take upper and lower)
Then acording to each function letter, call the correct function
If the character is not valid, prompt to ask again

Function Defintions:
std::vector<int> DisplayElements(char)
{
If the vector is empty, print an empty list []
Else Iterate through the vector and cout each eement(Make sure to use brackets)
}
